...
│   ├── field.js           force layout, walker, traversal state
│   ├── plain.js           the fallback view
│   ├── strata.js          the stratigraphic section
│   ├── tunnel.css         site styles; the locked tokens are linked from the
│   │                      Tunnel skill's CDN, never inlined
│   └── tunnel-atavistic.css   Tunnel's first-commit tokens, kept for `revert`
└── index.html
```

The site is static files with no build step: `index.html` fetches
`data/words.yml`, parses it with `js-yaml` from a CDN, and routes between the
field (default), the stratigraphic section (`?section`), and the plain list
(`?plain`, automatic under `prefers-reduced-motion`).

## Build order

The order the site was built in, kept because it is also the dependency order
for changes:

1. `data/words.yml` complete. Provenance back-fill and attestation verification
   remain outstanding, below.
...
5. The stratigraphic section — drawn, and captioned as a sketch until the
   attestation dates are verified.
